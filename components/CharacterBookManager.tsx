...
import type { CharacterBook } from '@/types/types'
...
export function CharacterBookManager({
    ...
    const newBook: CharacterBook = {
      ...
      description:
        interfaceLanguage === 'zh'
          ? '关于角色的背景知识和世界观设定'
          : 'Background knowledge and worldbuilding about the character',
      scan_depth: 100,
      token_budget: 500,
  ...
  const deleteCharacterBook = () => {
    if (
      confirm(
        interfaceLanguage === 'zh' ? '确定要删除角色书吗？' : 'Are you sure you want to delete the character book?'
      )
    ) {
  ...
  const updateCharacterBookInfo = (field: keyof CharacterBook, value: any) => {
    ...
    onUpdateCharacterBook({ ...characterBook, [field]: value })
  }
  ...
  const addNewEntry = () => {
    ...
    onUpdateCharacterBook({ ...characterBook, entries: [...characterBook.entries, newEntry] })
  }
  ...
  const deleteEntry = (index: number) => {
    ...
    onUpdateCharacterBook({ ...characterBook, entries: newEntries })
  }
      ...
      <Card>
        <CardHeader>
          <div className='flex justify-between items-center'>
            ...
            <Button onClick={deleteCharacterBook} variant='destructive' size='sm'>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder={
                interfaceLanguage === 'zh'
                  ? '描述这个角色书的内容和用途'
                  : 'Describe the content and purpose of this character book'
              }
              rows={2}
            />
          </div>
        ...
        <CardContent>
          {characterBook.entries.length === 0 ? (
            <div className='text-center text-muted-foreground py-8'>
              {interfaceLanguage === 'zh'
                ? '暂无条目，点击上方按钮添加'
                : 'No entries yet, click the button above to add'}
            </div>
          ) : (
            <div className='space-y-4'>
                ...
                <Card key={entry.id || index} className='border-l-4 border-l-blue-500'>
                  <CardHeader className='pb-2'>
                    <div className='flex items-center justify-between'>
                      ...
                      <div className='flex items-center space-x-2'>
                        ...
                        />
                        <Button onClick={() => deleteEntry(index)} variant='ghost' size='sm'>
                  ...
                  {expandedEntries.has(index) && (
                    <CardContent className='space-y-4'>
                      ...
                      <div>
                        <Label>
                          {interfaceLanguage === 'zh' ? '触发关键词 (逗号分隔)' : 'Trigger Keywords (comma separated)'}
                        </Label>
                        <Input
                          ...
                          placeholder={
                            interfaceLanguage === 'zh' ? '关键词1, 关键词2, 关键词3' : 'keyword1, keyword2, keyword3'
                          }
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          placeholder={
                            interfaceLanguage === 'zh'
                              ? '当关键词被触发时要插入的内容...'
                              : 'Content to insert when keywords are triggered...'
                          }
                          rows={4}
                        />
                      </div>
                      ...
                      {entry.selective && (
                        <div>
                          ...
                          <Input
                            ...
                            placeholder={
                              interfaceLanguage === 'zh' ? '次要关键词1, 次要关键词2' : 'secondary1, secondary2'
                            }
                          />
                        </div>
                      )}
