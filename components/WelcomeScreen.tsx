...
export function WelcomeScreen({
  ...
  const handleInterfaceLanguageChange = (newLang: 'zh' | 'en') => {
    ...
    // If the current model is not in the new language's model options, the new language's default model is used
      ...
      <Card className='w-full max-w-lg'>
        ...
        <CardContent className='space-y-4'>
          ...
          <div className='text-center mb-6'>
            <Button variant='outline' onClick={() => setIsVideoDialogOpen(true)} className='w-full sm:w-auto'>
              <Play className='w-4 h-4 mr-2' />
              {t.watchDemo}
            </Button>
            <p className='text-xs text-gray-500 mt-2'>{t.demoVideoDesc}</p>
          </div>

          <p className='text-center text-gray-600 mb-6'>{t.welcomeDesc}</p>
          ...
          <div>
            <Label htmlFor='welcome-apibaseurl'>
              {t.apiBaseUrl} ({t.optional})
            </Label>
            <Input
          ...
          <Button
            ...
            className='w-full'
            // disabled={!apiKey.trim()} // Removed to allow server-side fallback
          >
            {t.continue}
          </Button>
        </CardContent>
