...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Check if the ai role data folder already exists
    ...
    if (folderSearchResponse.data.files && folderSearchResponse.data.files.length > 0) {
      ...
      const folderResponse = await drive.files.create({
        requestBody: { name: 'AIRole_Characters', mimeType: 'application/vnd.google-apps.folder' },
      })
    ...
    // Create complete data packages with character data, pictures and chat history
    const fullData = {
      characterData,
    ...
    const media = { mimeType: 'application/json', body: JSON.stringify(fullData, null, 2) }
    ...
    if (existingFileResponse.data.files && existingFileResponse.data.files.length > 0) {
      ...
      response = await drive.files.update({ fileId, media })
    } else {
      // Create new file
      response = await drive.files.create({ requestBody: fileMetadata, media, fields: 'id' })
    }

    return NextResponse.json({ success: true, fileId: response.data.id, message: t.googleDriveMessages.saveSuccess })
  } catch (error) {
    console.error('Saving to Google Drive failed:', error)
